	...
	<div class="wrapper">
		...
		<div class="content-wrapper">
			...
			<div class="box-body table-responsive no-padding">
				<div class="row">
					<form action="getListOrderTourbyStateAdmin" method="get">
						<div class="col-md-3 form-group">
							<select class="form-control" name="stateMoney">
								...
								<c:if test="${requestScope.listTourDetail.size() >0 }">
									<c:forEach items="${requestScope.listTourDetail }" var="order">
										<tr>
											...
											<td>${order.note }</td>
											<td><a
												href="updateOrderTour.jsp?idOrder=${order.idOrder }&tourName=${order.tourName}"
												class="btn btn-info btn-xs"> <i class="fa fa-pencil"></i>
													Update
											</a> <s:url var="delete" action="deleteOrderAdmin">
					...
					<c:otherwise>
						<table class="table table-hover">
							<tbody>
								...
								<c:set value="" var="keyword"></c:set>
								<jsp:useBean id="orderDAO" class="adminDAO.OrderDAO"></jsp:useBean>
								<c:if test="${orderDAO.getAllOrders(keyword).size() >0 }">
									<c:forEach items="${orderDAO.getAllOrders(keyword) }"
										var="order">
										<tr>
											<td>${order.idOrder }</td>
											<td>${order.tourName }</td>
